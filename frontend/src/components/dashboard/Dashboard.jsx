...
export default function Dashboard() {
  ...
  // Sync sound toggle from Settings page
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    s.on('connect', () => { s.emit('get-status'); s.emit('get-initial-data'); });
    s.on('status', setWaStatus);
    ...
    s.on('initial-contacts', setContacts);
    s.on('new-message', (msg) => { s.emit('get-initial-data'); handleNewMessage(msg); });
    setSocket(s);
    return () => s.disconnect();
    ...
    <div className="dashboard">
      <div className={`sidebar ${mobileView === 'chat' ? 'sidebar-hidden-mobile' : ''}`}>
          ...
          <div className="sidebar-header-top">
            ...
            <div className="sidebar-actions">
              <button
                ...
                onClick={() => { const v = !soundEnabled; setSoundEnabled(v); localStorage.setItem('botora-notif-sound', v ? 'on' : 'off'); }}
              ...
              <div className={`wa-status-badge ${waStatus.isConnected ? 'connected' : 'disconnected'}`} title={waStatus.isConnected ? 'WhatsApp connecté' : 'WhatsApp non connecté'}>●</div>
        ...
        <div className="sidebar-content">
          {activePanel === 'chat' && (
            <ConversationList contacts={contacts} selectedContact={selectedContact} onSelectContact={handleSelectContact} onContactsUpdate={handleContactsUpdate} waStatus={waStatus} socket={socket} onConnectWhatsApp={() => socket?.emit('connect-whatsapp')} />
          )}
          {activePanel === 'stats' && <Stats socket={socket} />}
          {activePanel === 'faq' && <FAQManager />}
          {activePanel === 'config' && <BotConfig waStatus={waStatus} onConnectWhatsApp={() => socket?.emit('connect-whatsapp')} onLogoutWhatsApp={() => {}} />}
          {activePanel === 'settings' && <Settings />}
        </div>
      </div>

      <div className={`main-area ${mobileView === 'list' ? 'main-hidden-mobile' : ''}`}>
        <ChatWindow contact={selectedContact} socket={socket} waStatus={waStatus} onBack={handleBack} />
      </div>
